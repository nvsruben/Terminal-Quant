...
st.set_page_config(page_title="TERMINAL QUANTITATIF V31", layout="wide", initial_sidebar_state="expanded")
...
if not st.session_state.authentifie:
    ...
    st.markdown(
        ...
        "ACCÈS RESTREINT. V31 (ADVISORY & BRIEFING IA).</p>",
        unsafe_allow_html=True,
    )
...
# ==========================================
# DEVISES — Conversion FX vers EUR  [FIX BUG-4]
# ==========================================
DEVISE_PAR_TICKER = {
...
def obtenir_taux_change():
    """Retourne les taux de conversion vers EUR."""
    try:
        ...
        logger.warning(f"Impossible de récupérer les taux FX, fallback 1:1 — {e}")
        return {"USD": 1.0, "GBP": 1.0, "EUR": 1.0}


def devise_ticker(ticker: str) -> str:
    """Renvoie la devise d'un ticker. Par défaut USD pour les actions S&P 500."""
    return DEVISE_PAR_TICKER.get(ticker, "USD")


def prix_en_eur(prix: float, ticker: str, taux_fx: dict) -> float:
    """Convertit un prix dans sa devise d'origine vers EUR."""
    devise = devise_ticker(ticker)
    return prix * taux_fx.get(devise, 1.0)
...
# --- Mots-clés pour distinguer ETF/Crypto des actions individuelles [FIX BUG-2] ---
...
def est_etf_ou_crypto(nom_instrument: str) -> bool:
    nom_lower = nom_instrument.lower()
    return any(kw.lower() in nom_lower for kw in MOTS_CLES_ETF_CRYPTO)
...
def aspirer_le_marche_sp500():
    try:
        ...
        for t, n in zip(tickers, noms):
            ...
            dictionnaire_sp500[f"S&P500: {ticker_propre}"] = {
                "ticker": ticker_propre,
                "nom": n,
            }
        # [FIX FRAG-2] Vérification de cohérence
        if len(dictionnaire_sp500) < 400:
            logger.error(
                f"Scraping S&P 500 incomplet ({len(dictionnaire_sp500)} titres). "
                "Les données Wikipedia ont peut-être changé."
            )
        return dictionnaire_sp500
    except Exception as e:
        logger.error(f"Échec du scraping S&P 500 : {e}")
        return {}
...
def telecharger_donnees(liste_tickers):
    ...
    # [FIX PERF-5] 6 ans suffisent (3 ans de calcul + marge pour stress-tests 2020)
...
def obtenir_fondamentaux(ticker_str: str) -> dict:
    """[FIX PERF-3] Fondamentaux cachés 24 h pour éviter les appels réseau répétés."""
    try:
...
def analyser_sentiment_nlp():
    try:
        ...
        for item in toutes_les_news:
            ...
            lignes_news.append(
                {"Date": date_str, "Gros Titre": titre, "Score VADER": score}
            )
...
# ==========================================
st.sidebar.markdown(
    "<h3 style='font-family: monospace;'>CONTRÔLES SYSTÈME</h3>",
    unsafe_allow_html=True,
)
...
st.sidebar.markdown("---")
st.sidebar.markdown(
    "<h3 style='font-family: monospace;'>PARAMÈTRES DU PORTEFEUILLE</h3>",
    unsafe_allow_html=True,
)
budget = st.sidebar.number_input(
    "Capital Total Actuel (EUR)", min_value=10.0, value=950.0, step=10.0
)
...
# [FIX FRAG-3] Centroïdes initiaux fixés pour stabiliser les clusters
initial_centers = np.array(
    [
        [15.0, 1.5, 1.05, 0.002],   # Bull
        [22.0, 0.5, 1.00, -0.001],  # Neutre
        [35.0, -0.5, 0.95, -0.005], # Bear
    ]
)
...
credit_stress = float(df_brut["Stress_Credit"].iloc[-1]) < float(
    df_brut["Stress_Credit"].tail(50).mean()
)
...
dynamic_correl_max = (
    min(correl_max, 0.55) if tail_hedge_active or risk_score >= 40 else correl_max
)

# --- PRÉPARATION DES DONNÉES ---
df_hebdo = df_brut.tail(252 * 3).resample("W-FRI").last().dropna(how="all")  # [FIX PERF-6]
df_actifs = df_hebdo[
    [t for t in liste_tickers_bruts if t in df_hebdo.columns]
].copy()
...
max_dd = (
    (rendements_cumules - rendements_cumules.cummax()) / rendements_cumules.cummax()
).min()
...
correlation = pd.DataFrame(
    lw_cov_brut,
    index=rendements_propres.columns,
    columns=rendements_propres.columns,
).corr()
...
actifs_pre_eligibles = [
    a
    for a in univers_etudie.keys()
    if a in volatilite.index
    and not pd.isna(volatilite[a])
    and volatilite[a] <= 0.60
    and max_dd.get(a, -1.0) >= -0.45
    and a != "Bons du Trésor US 20A+"
]
top_20_candidats = (
    sortino_ajuste[actifs_pre_eligibles].sort_values(ascending=False).head(20).index.tolist()
)


# ==========================================
# 2. MACHINE LEARNING PRÉDICTIF [FIX BUG-1 + BUG-3 + STRUCT-1 + STRUCT-2]
# ==========================================
def construire_features_ml(candidat: str, rendements: pd.DataFrame, df_macro: pd.DataFrame) -> pd.DataFrame:
    """Construit un DataFrame de features enrichies pour un actif donné."""
    ret = rendements[candidat] if candidat in rendements.columns else None
    if ret is None:
        return pd.DataFrame()
    ...
    # Ajouter les indicateurs macro (réindexés sur les dates hebdomadaires)
...
for candidat in top_20_candidats:
    try:
        ...
        # [FIX STRUCT-2] Walk-forward cross-validation
        ...
        model.fit(X, y)
        derniere_ligne = X[-1].reshape(1, -1)  # [FIX BUG-1] reshape correct
        pred = model.predict(derniere_ligne)[0]
...
# ==========================================
# 3. MOTEUR DE SCORE MULTI-FACTEURS [FIX BUG-2 + PERF-3]
# ==========================================
fundamentals_data = []
...
for candidat in top_20_candidats:
    ...
    is_etf = est_etf_ou_crypto(nom_instrument)  # [FIX BUG-2]

    prix_brut = (
        float(df_brut[ticker_str].iloc[-1]) if ticker_str in df_brut.columns else 0
    )
    prix_actuel = prix_en_eur(prix_brut, ticker_str, taux_fx)  # [FIX BUG-4]

    sma_200_brut = (
        float(df_brut[ticker_str].tail(200).mean())
        if ticker_str in df_brut.columns
        else prix_brut
    )
    sma_200 = prix_en_eur(sma_200_brut, ticker_str, taux_fx)
    trend_ratio = (prix_actuel / sma_200) - 1.0 if sma_200 > 0 else 0

    pe, roe, consensus = 15.0, 0.15, 3.0

    if not is_etf:
        fondamentaux = obtenir_fondamentaux(ticker_str)  # [FIX PERF-3]
        ...
        if pe_raw is not None and 0 < pe_raw < 100:
            ...
            # PE invalide pour une action individuelle : on skip
            if pe_raw is not None and (pe_raw < 0 or pe_raw > 100):
                logger.info(f"PE invalide pour {candidat} ({pe_raw}), ignoré.")
                continue
...
if not df_zscore.empty:
    ...
    actifs_eligibles_finaux = (
        df_zscore.sort_values(by="Score_Global", ascending=False)["Actif"].tolist()
    )
else:
    actifs_eligibles_finaux = []

# --- Sélection top 5 décorrélés [FIX FRAG-5 — note: garde l'approche greedy mais sur données propres] ---
top_5_actifs = []
for candidat in actifs_eligibles_finaux:
    ...
    if candidat not in correlation.index:
        continue
    if not any(
        candidat in correlation.columns
        and s in correlation.index
        and correlation.loc[candidat, s] > dynamic_correl_max
...
# ==========================================
# 4. BLACK-LITTERMAN & ALLOCATION [FIX FRAG-4 + FRAG-8]
# ==========================================
...
if len(top_5_actifs) > 0 and budget_satellite > 0:
    rendements_top5 = rendements_propres[
        [a for a in top_5_actifs if a in rendements_propres.columns]
    ]
    if rendements_top5.shape[1] > 0:
        lw_cov_top5, _ = ledoit_wolf(rendements_top5)
        cov_matrix_normal = lw_cov_top5 * 52

        # Covariance sous stress (blend normal/crash)
        ...
        if len(idx_crash) > 10:
            actifs_valides_crash = [
                a for a in top_5_actifs if a in rendements_propres.columns
            ]
            cov_matrix_crash = (
                rendements_propres.loc[idx_crash, actifs_valides_crash].cov().values * 52
            )
            cov_matrix = 0.7 * cov_matrix_normal + 0.3 * cov_matrix_crash
        else:
            cov_matrix = cov_matrix_normal

        n_actifs = len(rendements_top5.columns)
        actifs_bl = rendements_top5.columns.tolist()

        inv_vol_bl = 1 / volatilite[actifs_bl].values
        w_eq = inv_vol_bl / inv_vol_bl.sum()
        Pi = 2.5 * np.dot(cov_matrix, w_eq)

        # [FIX FRAG-4] Vues basées sur les prédictions ML (quand disponibles)
        Q = np.array([ml_predictions.get(a, 0.0) * 52 for a in actifs_bl])
        # Confiance par vue : inversement proportionnelle à l'erreur CV du modèle
        omega_diag = []
        for a in actifs_bl:
            cv_err = abs(ml_cv_scores.get(a, -0.01))
            omega_diag.append(max(cv_err * 52, 0.01))
        ...
        try:
            term1 = inv(inv(tau * cov_matrix) + np.dot(P.T, np.dot(inv(Omega), P)))
            term2 = np.dot(inv(tau * cov_matrix), Pi) + np.dot(
                P.T, np.dot(inv(Omega), Q)
            )
            BL_returns = np.dot(term1, term2)

            poids_optimaux = np.clip(np.dot(inv(cov_matrix), BL_returns), 0, None)
            poids_optimaux = (
                poids_optimaux / poids_optimaux.sum()
                if poids_optimaux.sum() > 0
                else w_eq
            )

            # Plafonner les poids individuels
            for _ in range(10):
                ...
                if mask.sum() > 0:
                    poids_optimaux[mask] += excess * (
                        poids_optimaux[mask] / poids_optimaux[mask].sum()
                    )
                else:
                    break

            port_vol_initial = np.sqrt(
                np.dot(poids_optimaux.T, np.dot(cov_matrix, poids_optimaux))
            )
            expected_portfolio_return = np.dot(
                poids_optimaux, (rendements_hebdo[actifs_bl].mean() * 52).values
            )

            exposure_factor = min(
                1.0,
                target_volatility / port_vol_initial if port_vol_initial > 0 else 1.0,
            )
            ...
            allocations_satellite = pd.Series(
                poids_optimaux * budget_satellite_ajuste, index=actifs_bl
            )

        except Exception as e:
            logger.error(f"Optimisation Black-Litterman échouée : {e}")
            allocations_satellite = pd.Series(dtype=float)
...
if tail_hedge_active:
    allocations["Bons du Trésor US 20A+"] = (
        allocations.get("Bons du Trésor US 20A+", 0.0) + budget_tail_risk
    )
...
if len(allocations) > 1:
    try:
        ...
        actifs_valides_pca = [
            a for a in allocations.index if a in rendements_propres.columns
        ]
...
st.markdown(
    ...
    "padding-bottom: 10px;'>BUREAU CONSEIL & EXÉCUTION (V31)</h2>",
    unsafe_allow_html=True,
)
...
col2.metric(
    "POIDS CŒUR (SÉCURISÉ)",
    f"{capital_verrouille:.2f} €",
    delta="Sanctuarisé",
    delta_color="off",
)
col3.metric(
    "POIDS SATELLITE (ACTIF)",
    f"{budget_satellite:.2f} €",
    delta="Géré par l'IA",
    delta_color="normal",
)
col4.metric(
    "LIQUIDITÉS RECOMMANDÉES",
    f"{reserve_cash:.2f} €",
    delta=f"{(reserve_cash / budget) * 100:.1f}% de protection",
    delta_color="off",
)
...
# =================================================================
# ONGLET 1 : CONSEILLER STRATÉGIQUE
# =================================================================
with tab1:
    ...
    with col_ed1:
        ...
        config_colonnes = {
            "Actif": st.column_config.SelectboxColumn(
                "Instrument",
                options=list(univers_etudie.keys()),
                required=True,
            ),
            "Valeur (EUR)": st.column_config.NumberColumn(
                "Valeur Actuelle (€)",
                min_value=0.0,
                step=10.0,
                format="%.2f",
        ...
        # [FIX FRAG-6] Validation des saisies
        df_edited = df_edited.dropna(subset=["Actif"])
        df_edited = df_edited[df_edited["Actif"].isin(univers_etudie.keys())]
        df_edited = df_edited.drop_duplicates(subset=["Actif"], keep="first")
        df_edited["Valeur (EUR)"] = df_edited["Valeur (EUR)"].clip(lower=0)
        st.session_state.mon_portefeuille = df_edited.reset_index(drop=True)
    ...
    with col_ed2:
        ...
        if len(allocations) > 0:
            ...
            tous_actifs = set(allocations[allocations > 0].index.tolist()) | set(
                dict_actuel.keys()
            )
            for a in tous_actifs:
                val_cible = allocations.get(a, 0.0)
                val_actuelle = dict_actuel.get(a, 0.0)
                delta = val_cible - val_actuelle
                instrument_str = univers_etudie.get(a, {"nom": a})["nom"]
            ...
            if lignes_ordres:
                df_ordres = pd.DataFrame(lignes_ordres).sort_values(
                    by="Ordre Net (EUR)", ascending=False
                )
                st.dataframe(
                    ...
                    ).map(  # [FIX STRUCT-9] .map() au lieu de .applymap()
                        lambda x: (
                ...
                st.success(
                    "🎉 Votre portefeuille est parfaitement optimisé. "
    ...
    st.info(
        f"**Vision Macroéconomique :** Le logiciel détecte actuellement un "
        f"**{regime_marche}**. Le niveau de panique médiatique (NLP) est de "
        f"{avg_nlp_score:.2f}. Par conséquent, la consigne mathématique est de "
        f"conserver **{reserve_cash:.2f} € en liquidités (Cash)** pour respecter "
        f"votre limite de volatilité de {target_volatility * 100}%."
    )
    ...
    with col_conseil1:
        ...
        if len(a_vendre) == 0:
            st.write(
                "L'IA ne trouve aucune raison mathématique de vendre vos actifs "
                "non-verrouillés actuellement. Leur qualité est bonne."
            )
        else:
            for instrument in a_vendre:
                ticker = next(
                    (k for k, v in univers_etudie.items() if v["nom"] == instrument),
                    None,
                )
                if ticker:
                    z_score_val = (
                        df_zscore.loc[df_zscore["Actif"] == ticker, "Score_Global"].values[0]
                        if not df_zscore.empty and ticker in df_zscore["Actif"].values
                        else -99
                    )
                    if z_score_val == -99:
                        st.write(
                            f"- **{instrument} :** Ses fondamentaux ou son momentum se sont "
                            "trop dégradés. L'IA le rejette de la matrice d'élite."
                        )
                    elif z_score_val < 0:
                        st.write(
                            f"- **{instrument} :** Son Score de Qualité (Z-Score) est "
                            f"négatif ({z_score_val:.2f}). Il perd sa force face à "
                            "d'autres opportunités."
                        )
                    else:
                        st.write(
                            f"- **{instrument} :** C'est un bon actif, mais l'IA estime "
                            "que vous en possédez *trop* par rapport au risque global. "
                            "Elle propose de prendre quelques profits."
                        )
    ...
    with col_conseil2:
        ...
        if len(a_acheter) == 0:
            st.write(
                "L'IA ne trouve aucune nouvelle opportunité qui justifie de payer "
                "les frais de transaction (Pénalité de Rotation active)."
            )
        else:
            for instrument in a_acheter:
                ...
                if ticker == "Bons du Trésor US 20A+":
                    st.write(
                        f"- **{instrument} :** Couverture d'Urgence ! Le marché est "
                        "dangereux, l'IA achète ceci pour protéger le portefeuille "
                        "en cas de krach imminent."
                    )
                elif ticker:
                    z_score_val = (
                        df_zscore.loc[df_zscore["Actif"] == ticker, "Score_Global"].values[0]
                    ...
                    pred_ml = (
                        df_zscore.loc[df_zscore["Actif"] == ticker, "Pred_ML"].values[0]
                        if not df_zscore.empty and ticker in df_zscore["Actif"].values
                        else 0
                    )
                    explication_ml = (
                        "Le Machine Learning prédit en plus une hausse future "
                        f"(CV score: {ml_cv_scores.get(ticker, 0):.4f})."
                        if pred_ml > 0
                        else "Ses bilans financiers sont excellents."
                    )
                    st.write(
                        f"- **{instrument} :** Fait partie de l'Élite. Score Global "
                        f"massif de **{z_score_val:.2f}**. {explication_ml}"
                    )


# =================================================================
# ONGLET 2 : MATRICE D'ALLOCATION CIBLE
# =================================================================
with tab2:
    st.markdown(
    ...
    col_kpi2.metric(
        ...
        f"{expected_portfolio_return / port_vol_initial if port_vol_initial > 0 else 0:.2f}",
    )
    col_kpi3.metric(
        "Boost Prédictif (IA)",
        "ACTIF" if any(v != 0 for v in ml_predictions.values()) else "INACTIF",
        "Gradient Boosting (9 features)",
    )
    col_kpi4.metric(
        "Covariance sous Stress",  # [FIX FRAG-8] Renommé — ce n'est pas de l'EVT
        "ACTIVE",
        "Blend Normal/Crash 70/30",
    )
    st.markdown("</div>", unsafe_allow_html=True)

    tous_actifs_tableau = list(
        dict.fromkeys(list(allocations.index) + top_20_candidats[:15])
    )
    donnees_tableau = []
    for a in tous_actifs_tableau:
        if a not in univers_etudie:
            continue
        statut = (
            "VERROUILLÉ (CŒUR)"
            if a in actifs_verrouilles
            else ("ALLOUÉ" if a in allocations and allocations[a] > 0 else "REJETÉ")
        )
        score_z = (
            f"{df_zscore.loc[df_zscore['Actif'] == a, 'Score_Global'].values[0]:.2f}"
        ...
        donnees_tableau.append(
            {
                "Instrument (Ticker)": (
                    f"{univers_etudie[a]['nom']} [{univers_etudie[a]['ticker']}]"
                ),
                "Statut": statut,
                "Score Qualité (Z)": score_z,
    ...
    if donnees_tableau:
        st.dataframe(
            pd.DataFrame(donnees_tableau)
            .sort_values(by="Capital Cible", ascending=False)
            .style.format({"Capital Cible": "{:.2f} €"})
            .map(  # [FIX STRUCT-9]
                lambda val: (
                    "background-color: #2c3e50;"
                    if "VERROUILLÉ" in str(val)
                    else (
                        "background-color: #1a4222;"
                        if "ALLOUÉ" in str(val)
                        else "color: #8b0000;"
                    )
                ),
                subset=["Statut"],
...
# =================================================================
# ONGLET 3 : ANALYSE DE RISQUE (PCA)
# =================================================================
with tab3:
    ...
    with col_pca1:
        ...
        st.write(
            "Identification mathématique des forces invisibles dirigeant le portefeuille."
        )
        st.metric(
            "Facteur Fantôme 1 (Risque Global)",
            f"{pca_explained_variance[0]:.1f}% de variance expliquée",
        )
        if len(pca_explained_variance) > 1:
            st.metric(
                "Facteur Fantôme 2 (Secteur/Style)",
                f"{pca_explained_variance[1]:.1f}% de variance expliquée",
            )
    with col_pca2:
        st.markdown("**Exposition Bêta Macroéconomique**")
        if len(allocations) > 0:
            tickers_alloc = [
                univers_etudie[a]["ticker"]
                for a in allocations.index
                if a in univers_etudie and univers_etudie[a]["ticker"] in df_brut.columns
            ]
            poids_alloc = np.array(
                [
                    allocations[a] / budget
                    for a in allocations.index
                    if a in univers_etudie and univers_etudie[a]["ticker"] in df_brut.columns
                ]
            )
            if len(tickers_alloc) > 0:
                ret_port = (
                    df_brut[tickers_alloc].pct_change().dropna() * poids_alloc
                ).sum(axis=1)
                ...
                if not df_beta.empty and len(df_beta) > 10:
                    betas = []
                    for f in ["S&P500", "Taux US", "Or"]:
                        var_f = df_beta[f].var()
                        beta = (
                            df_beta["Port"].cov(df_beta[f]) / var_f
                            if var_f > 0
                            else 0
                        )
                        betas.append(beta)
...
# =================================================================
# ONGLET 4 : STRESS-TESTS HISTORIQUES [FIX FRAG-7]
# =================================================================
with tab4:
    if len(allocations) > 0:
        ...
        if len(actifs_testes) > 0:
            ...
            def run_stress_test(
                ...
                """Exécute un stress-test et retourne le DataFrame du graphique + pire perte."""
                actifs_valides = [
                    a
                    for a in actifs
                    if univers_etudie[a]["ticker"] in df_period.columns
                    and df_period[univers_etudie[a]["ticker"]].isna().sum() < 5
                ]
                poids_valides = poids[actifs_valides]

                if len(actifs_valides) == 0 or poids_valides.sum() == 0:
                    return None, None, []

                poids_renorm = poids_valides / poids_valides.sum()
                tickers_valides = [univers_etudie[a]["ticker"] for a in actifs_valides]
                colonnes = tickers_valides + ["^GSPC"]
                ret = df_period[colonnes].pct_change().dropna()

                port_ret = (ret[tickers_valides] * poids_renorm.values).sum(axis=1)
                sp_ret = ret["^GSPC"]

                croissance_port = (1 + port_ret).cumprod() * 100
                croissance_sp = (1 + sp_ret).cumprod() * 100

                df_graph = pd.DataFrame(
                    {
                        "Ton Portefeuille (Moteur IA)": croissance_port,
                        "Indice S&P 500": croissance_sp,
                    }
                )
                pire_perte = croissance_port.min() - 100

                # [FIX FRAG-7] Actifs exclus du test
                actifs_exclus = [a for a in actifs if a not in actifs_valides]
                return df_graph, pire_perte, actifs_exclus

            col_st1, col_st2 = st.columns(2)

            # Scénario A : COVID
            df_covid = df_history.loc["2020-02-15":"2020-04-30"]
            graph_covid, perte_covid, exclus_covid = run_stress_test(
                df_covid, actifs_testes, poids_test, "COVID-19"
            )
            with col_st1:
                st.markdown(
                    "**SCÉNARIO A : Krach de Liquidité COVID-19 (Fév-Avr 2020)**"
                )
                if graph_covid is not None:
                    fig_covid = px.line(
                        graph_covid,
                        color_discrete_sequence=["#4a90e2", "#444444"],
                    )
                    ...
                    st.caption(
                        f"Pire perte subie par ton portefeuille : **{perte_covid:.1f}%**"
                    )
                    if exclus_covid:
                        st.warning(
                            f"⚠️ Actifs exclus (données insuffisantes en 2020) : "
                            f"{', '.join(exclus_covid)}"
                        )
                else:
                    st.warning("Données historiques insuffisantes pour 2020.")

            # Scénario B : Choc des Taux
            df_inf = df_history.loc["2022-01-01":"2022-10-31"]
            graph_inf, perte_inf, exclus_inf = run_stress_test(
                df_inf, actifs_testes, poids_test, "Taux 2022"
            )
            with col_st2:
                st.markdown(
                    "**SCÉNARIO B : Choc des Taux d'Intérêt (Jan-Oct 2022)**"
                )
                if graph_inf is not None:
                    fig_inf = px.line(
                        graph_inf,
                        color_discrete_sequence=["#4a90e2", "#444444"],
                    )
                    ...
                    st.caption(
                        f"Pire perte subie par ton portefeuille : **{perte_inf:.1f}%**"
                    )
                    if exclus_inf:
                        st.warning(
                            f"⚠️ Actifs exclus (données insuffisantes en 2022) : "
                            f"{', '.join(exclus_inf)}"
                        )
                else:
                    st.warning("Données historiques insuffisantes pour 2022.")

# --- Footer ---
st.markdown("---")
st.caption(
    f"Terminal Quantitatif V31 — Dernière exécution : "
