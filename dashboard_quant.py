...
st.set_page_config(page_title="TERMINAL QUANTITATIF V29", layout="wide", initial_sidebar_state="expanded")
...
if not st.session_state.authentifie:
    ...
    st.markdown("<p style='text-align: center; color: #888888; font-family: monospace;'>ACCÈS RESTREINT. V29 (ALPHA PROFOND & MOTEUR STOCHASTIQUE).</p>", unsafe_allow_html=True)
...
def generer_csv_europe(df_ordres):
    ...
    lignes = "".join([f"{date_jour};{row['Instrument']};{row['Action']};{str(round(abs(row['Ordre Net (EUR)']), 2)).replace('.', ',')}\n" for _, row in df_ordres.iterrows()])
...
budget = st.sidebar.number_input("Capital Total Actuel (EUR)", min_value=10.0, value=950.0, step=10.0, help="Valeur de tes positions + ton cash sur Trade Republic.")
seuil_vix = st.sidebar.slider("Seuil d'Alerte VIX (Panique)", 15, 40, 22)
target_volatility = st.sidebar.slider("Volatilité Annuelle Cible (%)", 5, 25, 12, help="Le logiciel ajustera le Cash pour ne jamais dépasser ce niveau de risque.") / 100.0
min_trade_size = st.sidebar.slider("Taille d'Ordre Minimum (EUR)", 10, 100, 50, help="Pour éviter de payer 1€ de frais sur des trop petites sommes.")
turnover_penalty = st.sidebar.slider("Pénalité de Rotation (%)", 5, 30, 15, help="La performance supplémentaire exigée pour justifier la vente d'une action actuelle.") / 100.0
...
with st.spinner(f'Matrice IA V29 en cours. Analyse par Gradient Boosting & PCA...'):
...
# --- 2. MACHINE LEARNING PRÉDICTIF (GRADIENT BOOSTING) ---
ml_predictions = {}
with st.spinner("Entraînement des Arbres de Décision (Gradient Boosting) sur le Top 20..."):
...
st.markdown("<h2 style='font-family: monospace; border-bottom: 1px solid #444; padding-bottom: 10px;'>TERMINAL PRIME BROKER (V29)</h2>", unsafe_allow_html=True)

col1, col2, col3, col4 = st.columns(4)
col1.metric("SCORE DE RISQUE SYSTÉMIQUE", f"{risk_score}/100", delta=regime_marche, delta_color="normal" if risk_score < 40 else "inverse")

sentiment_label = "Neutre"
sentiment_color = "off"
if avg_nlp_score > 0.15: 
    sentiment_label = "Euphorie"
    sentiment_color = "normal"
elif avg_nlp_score < -0.15: 
    sentiment_label = "Peur/Panique"
    sentiment_color = "inverse"

col2.metric("SENTIMENT DE L'ACTUALITÉ (NLP)", f"{avg_nlp_score:.2f}", delta=sentiment_label, delta_color=sentiment_color)
col3.metric("SATELLITE TACTIQUE (IA)", f"{budget_satellite:.2f} EUR", delta=f"Limite Covariance : {dynamic_correl_max*100:.0f}%", delta_color="normal")
col4.metric("CASH (CIBLAGE VOLATILITÉ)", f"{reserve_cash:.2f} EUR", delta=f"{(reserve_cash/budget)*100:.1f}% (poids dynamique)", delta_color="off")

tab1, tab2, tab3, tab4, tab5 = st.tabs(["GESTION DES ORDRES (OMS)", "MATRICE D'ALLOCATION CIBLE", "ANALYSE FACTORIELLE & PCA", "MONTE CARLO STOCHASTIQUE", "TABLEAU DE BORD MACRO & NLP"])
    ...
    with col_ed2:
        ...
        if len(allocations) > 0:
            ...
            for a in set(allocations[allocations > 0].index.tolist() + list(dict_actuel.keys())):
                ...
                action = "ACHETER" if delta > 0 else "VENDRE" if abs(delta) >= min_trade_size else "CONSERVER (Trop Petit)"
            ...
            if lignes_ordres:
                ...
                st.dataframe(df_ordres.style.format({"Cible": "{:.2f} €", "Actuel": "{:.2f} €", "Ordre Net (EUR)": "{:+.2f} €"}).applymap(lambda x: 'color: #00cc00;' if x == 'ACHETER' else ('color: #ff4b4b;' if x == 'VENDRE' else 'color: #888888;'), subset=['Action']), use_container_width=True)
            else: st.success("Portefeuille parfaitement aligné.")
...
with tab2:
    ...
    col_kpi3.metric("Boost Prédictif (Machine Learning)", "ACTIF", "Arbres de Décision Gradient Boosting")
    col_kpi4.metric("Matrice Risque Extrême (EVT)", "ACTIVE", "Covariance sous Stress")
    st.markdown("</div>", unsafe_allow_html=True)

    donnees_tableau = [{"Instrument (Ticker)": f"{univers_etudie[a]['nom']} [{univers_etudie[a]['ticker']}]", "Statut": "VERROUILLÉ (CŒUR)" if a in actifs_verrouilles else ("ALLOUÉ" if a in allocations and allocations[a]>0 else "REJETÉ"), "Score Z": f"{df_zscore.loc[df_zscore['Actif']==a, 'Score_Global'].values[0]:.2f}" if not df_zscore.empty and a in df_zscore['Actif'].values else "N/A", "Capital Cible": allocations.get(a, 0.0)} for a in dict.fromkeys(list(allocations.index) + top_20_candidats[:15])]
...
with tab3:
    ...
    with col_pca1:
        ...
        st.metric("Facteur Fantôme 1 (Risque de Marché)", f"{pca_explained_variance[0]:.1f}% de variance expliquée")
        if len(pca_explained_variance) > 1: st.metric("Facteur Fantôme 2 (Style/Secteur)", f"{pca_explained_variance[1]:.1f}% de variance expliquée")
    with col_pca2:
...
with tab4:
    st.markdown("**Mouvement Brownien Géométrique (1000 Scénarios, 252 Jours)**")
    if len(allocations) > 0 and budget > 0 and port_vol_initial > 0:
        ...
        fig_mc.add_hline(y=np.percentile(sims[-1], 5), line_dash="dash", line_color="#ff4b4b", annotation_text="Pire Perte (VaR 95%)")
        fig_mc.update_layout(template="plotly_dark", height=400, margin=dict(t=0,b=0,l=0,r=0))
        st.plotly_chart(fig_mc, use_container_width=True)
    else: st.warning("Allocation insuffisante pour exécuter la simulation stochastique.")

with tab5:
    if not df_headlines.empty:
        st.dataframe(df_headlines.style.applymap(lambda v: 'color: #00cc00;' if v>0.15 else ('color: #ff4b4b;' if v<-0.15 else 'color: #888888;'), subset=['Score VADER']).format({"Score VADER": "{:.2f}"}), use_container_width=True, height=200)
    col_m1, col_m2 = st.columns(2)
    with col_m1: st.line_chart(pd.DataFrame({"Écart de Taux (10A-3M)": (df_brut['^TNX'] - df_brut['^IRX']).tail(252)}), color="#8b0000" if curve_inverted else "#4a90e2")
    with col_m2: st.line_chart(pd.DataFrame({"Stress de Crédit Interbancaire": df_brut['Stress_Credit'].tail(252)}), color="#8b0000" if credit_stress else "#4a90e2")
